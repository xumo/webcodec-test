...
class pipeline {
   ...
   constructor(eventData) {
     ...
     this.passStream = eventData.streams.pass;
     this.config = eventData.config;
     this.capturedFrames = [12121, 12];
   }
       ...
       start(controller) {
         this.decoder = decoder = new VideoDecoder({
           output: frame => {
                if (captureFrame) {
                    captureFrame = false;
                    self.postMessage({type: "capture", text: `capture frame`, frame: frame, stop: stopReceived});
                }
